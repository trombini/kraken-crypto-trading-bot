...
export class BuyAnalyst extends Analyst {
  ...
  constructor(readonly watcher: AssetWatcher, readonly config: BotConfig) {
    ...
    this.registerIndicator(true, 0.1, 15, 'UPSWING 15m', upswing('15m', 15, config.blockMaturity))
    this.registerIndicator(false, 0.35, 240, 'UPTREND 4h', uptrend('4h', 240, 0.5))
    this.registerIndicator(false, 0.35, 240, 'RSI 4h', rsi('4h'))
    this.registerIndicator(false, 0.20, 240, 'STOCHF 4h', stochastic('4h'))
